app.controller("controller.home.blog", function($scope , $state , $users ,  $stateParams , $http , Flash , $uibModal) {
  ...
  $scope.editor = {source : '' , tags : [] , title : '' , header : '' , mode : 'header' , shortUrl : '', ogimage : emptyFile , ogimageUrl : '' , description : '', tagsCSV : '' ,section : '' , author : '',ogvideo:emptyFile};
  ...
  if (typeof $stateParams.id != 'undefined' && $stateParams.action == 'edit') {
    ...
    then(function(response){
      ...
      $scope.editor.ogvideo = response.data.ogvideo
    ...
    setup: function (editor ) {
      ...
      editor.addButton( 'publishBtn', {
        ...
        onclick: function() {
          ...
          if ($scope.editor.ogvideo != emptyFile && typeof $scope.editor.ogvideo != 'string' && $scope.editor.ogvideo != null ) {
            fd.append('ogvideo' , $scope.editor.ogvideo);
          ...
          fd.append('language' , $scope.editor.lang);
      ...
      editor.addButton( 'saveBtn', {
        ...
        onclick: function() {
          ...
          var svd = new FormData();

          svd.append('source' ,$scope.editor.source);
          svd.append('header' ,$scope.editor.header);
          svd.append('title' ,$scope.editor.title);
          svd.append('users' ,[$scope.me.pk]);
          svd.append('sourceFormat' ,'html');
          svd.append('state' ,'saved');
          svd.append('tags' , tags);

          if ($scope.editor.ogimage ==emptyFile) {
            Flash.create('danger' , 'Add Image')
            return;
          }
          else{
            svd.append('ogimage' , $scope.editor.ogimage);
          }


          if ($scope.editor.ogvideo != emptyFile && typeof $scope.editor.ogvideo != 'string' && $scope.editor.ogvideo != null ) {
            svd.append('ogvideo' , $scope.editor.ogvideo);
          ...
          if ($scope.editor.shortUrl == '' ) {
            Flash.create('danger' , 'Please Add Youtube Video URL');
            return;
          }
          if ($scope.editor.language == '' ) {
            Flash.create('danger' , 'Please Select Language');
            return;
          }
        
          svd.append('ogimageUrl' , $scope.editor.ogimageUrl);
          svd.append('shortUrl' , $scope.editor.shortUrl);
          svd.append('tagsCSV' , $scope.editor.tagsCSV);
          svd.append('section' , $scope.editor.section);
          svd.append('author' , $scope.editor.author);
          svd.append('language' , $scope.editor.lang);
          svd.append('description' , $scope.editor.description);
          if ($scope.mode == 'edit') {
            method = 'PATCH';
          ...
          $http({method : method , url : url, data : svd, transformRequest: angular.identity,
          headers: {
            'Content-Type': undefined
          }}).
          then(function(response){
            ...
            $scope.editor.mode = 'hedaer';
            $scope.editor.ogimage = emptyFile;
            $scope.editor.ogvideo = emptyFile;
          }, function(response){
            Flash.create('danger' , response.status + ' : ' + response.statusText);
          });
        }
